...
// File contracts/ERC721AutoId.sol
 ...
 */
contract ERC721AutoId is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable {
     ...
     */
    function mint(address to) public virtual returns (uint tokenId){
...
contract X is ERC721AutoId {

    event Mint(uint indexed id, uint indexed tokenType);

    mapping(uint => uint) private _types;

    constructor() ERC721AutoId("X", "X", "") {
    }

    function tokenType(uint id) public view returns(uint) {
        return _types[id];
    }

    function mint(address to, uint _type) public {
        uint id = super.mint(to);
        _types[id] = _type;

        emit Mint(id, _type);
    }

    function mint(address) public override pure returns(uint) {
        revert("disabled");
    }
}
...
contract Y is ERC721AutoId {

    constructor() ERC721AutoId("Y", "Y", "") {
    }
}


// File contracts/WestWorld.sol
...
contract WestWorld {
    ...
    uint public constant TYPE_COUNT = 4;
    X public x;
    Y public y;
    ...
    function request() public {
        x.mint(msg.sender, randomType());
    }

    function combine(uint[TYPE_COUNT] memory ids) public {
        for (uint256 index = 0; index < TYPE_COUNT; index++) {
            uint id = ids[index];
            uint _type = x.tokenType(id);
            require(_type == index, "wrong tokens");
            x.burn(id);
        }
        y.mint(msg.sender);
    }

    function randomType() public view returns (uint) {
        return x.totalSupply() * block.timestamp % TYPE_COUNT;
    }
